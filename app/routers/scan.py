...
# Global dictionary to track active build pipeline stages without modifying the database schema.
ACTIVE_SCANS = {}
...
async def _run_mock_simulation(scan_run_id: int, stages: list) -> None:
    try:
        # Stage 1: Repo Cloning (started as completed already)
        update_in_memory_stage(scan_run_id, "clone", stages)
        await asyncio.sleep(2)
        
        # Stage 2: Secret Scanning Stage (Parallel)
        ...
        # Stage 3: Findings Processing Stage
        ...
        # Stage 4: Code Context Extraction
        ...
        # Stage 5: AI Validation Stage (Llama 3.1)
        ...
        # Stage 6: Committer Information Collection
        ...
        # Stage 7: Storage & Alert Dispatch
...
async def _poll_jenkins_freestyle_pipeline(scan_run_id: int, queue_url: str, auth: str, stages: list) -> None:
    try:
        ...
        # Poll queue for up to 60 seconds (30 * 2s)
        for _ in range(30):
        ...
        elapsed_seconds = 0
        current_active_idx = 1  # starts at index 1 ('scan')
        stages[0]["status"] = "completed"
        stages[0]["time"] = "3s"
        stages[1]["status"] = "running"
        stages[1]["time"] = "In progress..."
        ...
        # Stage estimated durations in seconds for time-based progress synthesis
        stage_times = {
            "scan": 12,
            "process": 4,
            "extract": 3,
        ...
        while True:
            ...
            # Fetch latest state from in-memory cache in case a callback hook advanced it!
            in_mem = ACTIVE_SCANS.get(scan_run_id)
            if in_mem:
                stages = in_mem.get("stages") or stages
                
            # Check active stage index from memory
    ...
    try:
        ...
        if settings.jenkins_url and settings.jenkins_job_name:
            ...
            if queue_url:
                ...
                # Mock fallback if queue location header isn't supplied
...
async def trigger_scan(
    ...
    try:
        ...
        await _run_jenkins_or_mock(scan_run_id, repo_url, branch_names)
...
def get_run(scan_run_id: int, _user: dict = Depends(get_current_user)):
    ...
    if in_mem:
        ...
        # Synthesize clean stage timeline for completed or failed historical runs
        status = row["status"]
        stages = [
        ...
        if status == "failed":
            # For historical failed runs, set the stages after "extract" as failed
            for stg in stages:
            ...
            # If for some reason it's running but not in memory (e.g. server restarted), synthesize a starting/running state
            for idx, stg in enumerate(stages):
                if idx == 0:
                    stg["status"] = "completed"
                    stg["time"] = "Done"
...
def update_run_stage(
    ...
    stages = in_mem.get("stages") if in_mem else []
    if not stages:
        # If not active in memory, initialize default stages to allow manual/webhook updates
        stages = [
            {"id": "clone", "name": "Repository Clone Stage", "status": "completed", "desc": "Jenkins clones GitHub repository to temporary workspace", "time": "Just now"},
            {"id": "scan", "name": "Secret Scanning Stage (Parallel)", "status": "pending", "desc": "TruffleHog, Gitleaks, Nosey Parker parallel scan", "time": ""},
            {"id": "process", "name": "Findings Processing Stage", "status": "pending", "desc": "Consolidating findings & de-duplicating results", "time": ""},
    ...
    for stg in stages:
        if stg["id"] == body.stage:
            ...
            # Auto-complete previous running stage to completed
            stg["status"] = "completed"
            stg["time"] = "Done"
